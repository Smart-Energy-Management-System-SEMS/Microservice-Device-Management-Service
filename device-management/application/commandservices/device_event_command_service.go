...
func (s *DeviceEventCommandService) RecordEvent(ctx context.Context, command commands.RecordDeviceEventCommand) (*entities.DeviceEvent, error) {
	...
	s.eventHandler.Publish(ctx, s.eventHandler.Topics().DeviceEvents, eventhandlers.EventTypeDeviceEventRecorded, event.DeviceID, device.UserID, map[string]interface{}{
